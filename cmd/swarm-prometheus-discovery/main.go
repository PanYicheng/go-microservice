...
var cfg struct {
	...
	IgnoredServiceStr string `default:"prometheus,grafana" arg:"env:IGNORED_SERVICE_STR" help:"Comma-separated list of service names we do not want to scrape"`
}
...
func main() {
	// p, err := arg.NewParser(arg.Config{IgnoreEnv:false}, &cfg)
	// err = p.Parse(flags())
	arg.MustParse(&cfg)

	logrus.Infof("networkName: %v\n", cfg.NetworkName)
	logrus.Infof("ignoredServicesStr: %v\n",cfg.IgnoredServiceStr)
    logrus.Println("Starting Swarm-scraper!")
    // Stuff any services we don't want to scrape (such as ourselves and prometheus server)
    // into a slice.
    parseIgnoredServices(cfg.IgnoredServiceStr)
    ...
    findNetworkId(dockerClient, cfg.NetworkName)
